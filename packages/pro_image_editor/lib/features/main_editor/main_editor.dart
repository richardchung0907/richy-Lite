...
class ProImageEditorState extends State<ProImageEditor>
  ...
  void openTuneEditor({bool enableHero = true}) async {
    ...
    // Merge: keep only the LATEST value per adjustment id.
    // Prevents the append-only history from accumulating duplicates
    // that cause slider drift and matrix stacking.
    final merged = <String, TuneAdjustmentMatrix>{};
    for (final item in stateManager.activeTuneAdjustments) {
      merged[item.id] = item.copy();
    }
    for (final item in tuneAdjustments) {
      merged[item.id] = item.copy();
    }
  ...
  void openFilterEditor() async {
    ...
    FilterState? filterState = await openPage(
      FilterEditor.autoSource(
        ...
        initConfigs: FilterEditorInitConfigs(
          ...
          appliedFilters: stateManager.activeFilters.allMatrices,
          appliedTuneAdjustments: stateManager.activeTuneAdjustments,
          activeFilterName: stateManager.activeFilters.isNotEmpty ? stateManager.activeFilters.last.name : null,
          activeFilterOpacity: stateManager.activeFilters.isNotEmpty ? (stateManager.activeFilters.last.meta['opacity'] as double?) : null,
        ),
      ),
    );
    ...
    if (configs.filterEditor.enableMultiSelection) {
      addHistory(
        filters: [
