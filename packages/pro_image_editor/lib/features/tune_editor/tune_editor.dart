...
class TuneEditorState extends State<TuneEditor>
  ...
  /// Persists the last selected tab across TuneEditor instances so the user
  /// returns to the same tab (brightness/contrast/saturation) on re-entry.
  /// Package-private so `close()` can also update it.
  static int lastSavedTabIndex = 0;
  ...
  void initState() {
    super.initState();
    // Restore the last selected tab (brightness/contrast/saturation)
    selectedIndex = lastSavedTabIndex;
  ...
  /// editor.
  void done() async {
    // Save the selected tab before closing, so it's restored on re-entry
    lastSavedTabIndex = selectedIndex;
