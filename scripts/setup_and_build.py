...
Usage:
    python scripts/setup_and_build.py              # Full auto-setup + build
    python scripts/setup_and_build.py --fresh      # Force re-install everything
    python scripts/setup_and_build.py --check-only # Only check environment
    python scripts/setup_and_build.py --hot-reload # Skip to Hot Reload directly
"""
...
def get_flutter_url() -> str:
    """Return the Flutter download URL."""
...
def main():
    ...
    # Use the scripts/ subdirectory location for build_android.py
    build_script = os.path.join(PROJECT_DIR, "scripts", "build_android.py")
    ...
    log("  Next time, just run: python scripts/setup_and_build.py")
    log("=" * 60)
