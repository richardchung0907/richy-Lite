...
Usage:
    python scripts/build_android.py                  # Full check + build debug APK
    python scripts/build_android.py --analyze        # Only static analysis
    python scripts/build_android.py --release        # Build release APK
    python scripts/build_android.py --hot-reload     # Launch `flutter run` with Hot Reload
    python scripts/build_android.py --install        # Build + auto-install on device
    python scripts/build_android.py --clean          # Clean stale build artifacts first
"""
...
# Reconfigure stdout/stderr to UTF-8 on Windows to prevent UnicodeEncodeErrors with terminal symbols
...
def _clean_pub_state():
    """Remove pubspec.lock and .dart_tool/."""
