...
Usage:
    python run_app.py              # Full flow: check → launch → build → run
    python run_app.py --only-build # Build APK only (no emulator, no run)
    python run_app.py --only-run   # Skip build, just launch & hot reload
    python run_app.py --cold       # Cold start emulator (wipe data)
"""
